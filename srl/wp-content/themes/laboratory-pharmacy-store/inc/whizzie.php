...
if (!$online_pharmacy_menu_exists) {
    ...
    if (!has_nav_menu($online_pharmacy_bpmenulocation)) {
        ...
        for ($i = 1; $i <= 4; $i++) {
         ...
         // Product Section //
    set_theme_mod('laboratory_pharmacy_store_about_title', 'Featured Products');
    set_theme_mod('laboratory_pharmacy_store_best_product_category', 'Product Category');
    ...
    foreach ($online_pharmacy_category_names as $online_pharmacy_index => $online_pharmacy_category_name) {
        ...
        for ($online_pharmacy_i = 0; $online_pharmacy_i < 4; $online_pharmacy_i++) {
            ...
            update_post_meta($online_pharmacy_post_id, '_sale_price', 120); // Sale price
            update_post_meta($online_pharmacy_post_id, '_regular_price', 200); // Regular price
            update_post_meta($online_pharmacy_post_id, '_price', 200); // Current price (use sale price)
